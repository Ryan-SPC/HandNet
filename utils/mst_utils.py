    ...
    for i in range(len(joint_pos)):
        ...
        for j in range(i+1, len(joint_pos)):
            ...
            if outside_bone_sample > 1:
                cost_matrix[i, j] = 5 * outside_bone_sample
                cost_matrix[j, i] = 5 * outside_bone_sample
            threshold = 0.2
            rate = np.linalg.norm(joint_pos[i]-joint_pos[j])
            

            cost_matrix[i,j] += rate
            cost_matrix[j,i] += rate
