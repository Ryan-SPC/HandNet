#-------------------------------------------------------------------------------
# Name:        Wrist_GCN.py
# Purpose:     definition of wrist prediction module.
...
class WRISTNET(torch.nn.Module):
    def __init__(self):
        super(WRISTNET, self).__init__()
        self.shape_encoder = ShapeEncoder()
        self.joint_encoder = JointEncoder()
        self.back_layers = Sequential(MLP([128 + 128, 200, 64]), Linear(64, 3))
    ...
    def forward(self, data, shuffle=True):
        ...
        for i in range(len(torch.unique(data.joints_batch))):
            ...
            left = data.left_wrist[(i*3):(i*3+3)]
            right = data.right_wrist[(i*3):(i*3+3)]
            label_i = joint_i.new(torch.Size((joint_i.shape[0], 1))).zero_()
            for i in range (len(label_i)):
                if np.array_equal( joint_i[i].data.cpu().detach().numpy() , left.data.cpu().detach().numpy()):
                    label_i[i,0] = 1
                elif np.array_equal( joint_i[i].data.cpu().detach().numpy() , right.data.cpu().detach().numpy()):
                    label_i[i,0] = 2
                else:
                    label_i[i,0] = 0

            # random shuffle
            shuffle_flag = joint_i.new(torch.Size((joint_i.shape[0], 1))).zero_()
            if shuffle:
                idx = torch.randperm(shuffle_flag.nelement())
                label_i = label_i[idx]
                joint_i = joint_i[idx]
